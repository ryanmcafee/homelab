...
| Everything else (API tokens, OAuth clients, service credentials, the Alertmanager Slack webhook and Pushover keys) | 1Password vault `homelab` | `OnePasswordItem` resources rendered by the `*-config` / `*-dependencies` child charts (Alertmanager: `prometheus-config`, `docs/runbooks/alerting.md`) |
