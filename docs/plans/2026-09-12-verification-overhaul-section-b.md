...
**Tech Stack:** TypeScript scripts on Bun, Go (cobra, `internal/verify.Runner`), Helm 4.2.0, kind v0.33.0 + `kindest/node:v1.36.1`, Cilium (chart pin in versions.yaml), ArgoCD CLI v3.5.2, Chainsaw v0.2.15, conftest, GitHub Actions.
...
- TypeScript (Bun) for every script; no Bash/Python scripts (ADR-005). Shebang `#!/usr/bin/env bun`, `--help`, `--dry-run` where destructive, colour log helpers (cyan INFO, green OK, red ERROR, yellow WARN), exit 0/1. Pure logic in exported functions with `scripts/<name>_test.ts` (`task test:scripts` and `task scripts:lint` run in CI).
...
| Task | Command |
|---|---|
| `localdev:kind` | `bun scripts/localdev-kind.ts up` |
...
- [ ] **B6 Tests + lint.** `bun test scripts/localdev-kind_test.ts` (hosts.toml rendering, upstream table, version parsing from a YAML string, kind image tag derivation); `task scripts:lint`. Do NOT create a Kind cluster from the subagent; the integrator runs the real loop.
...
- [ ] **C4 Verify.** `task verify:text -- --chart bootstrap` (snapshot failure for bootstrap expected — do not update), `conftest`/policy unaffected, `task scripts:lint` and `task test:scripts` on the new script.
...
- [ ] **E6 Tests + lint.** `bun test scripts/localdev-argocd_test.ts`, `task scripts:lint`. `tilt alpha tiltfile-result` is not required; `tilt ci --help` unaffected. Do NOT run against a cluster from the subagent.
...
- [ ] **H1 Workflow.** Jobs: `kind-direct` (Tilt direct mode; uses `task localdev:kind` so Cilium is present; `tilt ci --timeout 15m`; asserts as before) and `kind-argocd` (**required, no continue-on-error**, `timeout-minutes: 45`): checkout, setup-go, install pinned tools from versions.yaml-mirrored `env:` with renovate markers (kind, kubectl, helm, argocd, chainsaw, bun, task, tilt), `actions/cache` on `~/.cache/homelab-kind-registry` keyed `kind-registry-${{ hashFiles('configuration/versions.yaml', 'charts/addons/values-localdev.yaml', 'charts/applications/values-localdev.yaml') }}` with `restore-keys: kind-registry-`, `task localdev:ci`, then `task verify LEVEL=2 | tee verify-level2.json` + step summary, `if: always()` → `task localdev:diagnose` and upload `verify-level2.json`. Triggers add `scripts/**`, `configuration/**`, `tests/e2e/**`, `tests/health/**`, `Taskfile.yml`, `cmd/**`, `internal/**`, `mise.toml`. `yaml-lint` job: drop `|| true`. `verify.yml`: add `task test:health` to the `policy` job (needs the argocd CLI pinned from `tools.argocd`).
...
1. `task config:export:localdev`, `task test:snapshot -- --update`, `task verify:text`, `go test ./...`, `task test:policy`, `task test:health`, `task scripts:lint && task test:scripts`.
