...
`paperclip-api-keys` as optional environment variables, and `adapters.apiKeys.enabled` (default
`false`) decides whether the operator also injects `ANTHROPIC_API_KEY` / `OPENAI_API_KEY`, see [Agent credentials](#agent-credentials-subscriptions-or-api-keys).
...
| `paperclip-api-keys` | `op://homelab/paperclip-api-keys` | `CLAUDE_CODE_OAUTH_TOKEN` (subscription, the default); `ANTHROPIC_API_KEY`, `OPENAI_API_KEY` only with `adapters.apiKeys.enabled: true` | the chart's `adapters.extraSecretEnv` (`CLAUDE_CODE_OAUTH_TOKEN`); `spec.adapters.apiKeysSecretRef` (`ANTHROPIC_API_KEY`, `OPENAI_API_KEY`) when `adapters.apiKeys.enabled` |
...
container, so a missing key is simply unset. The operator's `spec.adapters.apiKeysSecretRef`, which
adds `ANTHROPIC_API_KEY` and `OPENAI_API_KEY` as optional `secretKeyRef` env on the server
(operator 0.19.1 `internal/resources/statefulset.go`), is rendered only with
`adapters.apiKeys.enabled: true` (default `false`): subscriptions are the default, and with them the
two API-key variables do not exist in the pod at all. The 1Password item carries whichever you use.

### API keys (API billing)

Set `adapters.apiKeys.enabled: true` in `charts/paperclip/values.yaml` (the operator then wires
both variables from the item), then:
...
`adapters.apiKeys.enabled` at `false`, so no `ANTHROPIC_API_KEY` reaches the pod (an API key would
take precedence and bill the API). Renew the
...
With `adapters.apiKeys.enabled: false` no `OPENAI_API_KEY` reaches the pod either, so Codex bills
...
Kind seeds all three keys as placeholders in `localdev/fakes/secrets.yaml`; with the default
`adapters.apiKeys.enabled: false` only `CLAUDE_CODE_OAUTH_TOKEN` is wired, and flipping the toggle in
Kind exercises the operator path too. No agent runs there.
