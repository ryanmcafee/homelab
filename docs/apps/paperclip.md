...
## Agent credentials: subscriptions or API keys
...
`spec.adapters.apiKeysSecretRef` injects only `ANTHROPIC_API_KEY` and `OPENAI_API_KEY` (optional
...
entry for the server and the onboarding init container, so a missing key is simply unset. Nothing
forces an API key: both agents can run on a subscription.

**Claude (`claude_local`)**, recommended: a subscription OAuth token. On a workstation logged into
the Pro/Max/Team/Enterprise account:

```bash
claude setup-token      # opens the browser, prints a one-year token, saves nothing
```

Store the token as field `CLAUDE_CODE_OAUTH_TOKEN` of the 1Password item `paperclip-api-keys` and
leave `ANTHROPIC_API_KEY` out of the item: Claude Code ranks an API key above
`CLAUDE_CODE_OAUTH_TOKEN`, and the key bills the API instead of the subscription. Renew the token
yearly. Alternative: log in inside the pod, which writes `/paperclip/.claude/.credentials.json` on
the PVC; such logins expire and are renewed with `/login`, and `/status` shows which credential is
active. Paperclip's "Test Environment" button on the agent reports the auth mode it detected.

```bash
kubectl -n paperclip exec -it paperclip-0 -- claude   # choose the Claude account login, paste the code the browser shows
```

**Codex (`codex_local`)**: no API key needed. A ChatGPT login lives in `~/.codex/auth.json`, Codex
refreshes it automatically and Paperclip symlinks the file into each agent's managed `CODEX_HOME`.
Seed it once inside the pod (device-code login must be enabled in the ChatGPT account's security
settings; the CLI prints a code to enter in the browser):
...
Do not put `OPENAI_API_KEY` in the 1Password item when using the subscription: a per-agent key
overrides the host login and bills API rates. The operator's NetworkPolicy already allows egress on
TCP 443, which the logins and the models need. `auth.json` is a credential; never commit or share it.
...
item back to `ANTHROPIC_API_KEY`.

Kind seeds `CLAUDE_CODE_OAUTH_TOKEN` (a placeholder) instead of `ANTHROPIC_API_KEY` in
`localdev/fakes/secrets.yaml`; no agent runs there.
