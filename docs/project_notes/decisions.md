...
- **Requiring the merge-result check closes the window at run time only, and that is not the whole fix.** `verify.yml` verifies the merge result *as of when it last ran*, and `pull_request` fires on head events only, so a base advance never re-triggers it: the green persists while `main` moves underneath, and once the context is required that stale green is a *satisfied* requirement. The collision above walks straight through — PR X green against `main@T1`, PR Y merges `### ADR-034:` at T2, PR X is one behind and `CLEAN` with a green required level 0, duplicate lands. Making the verdict current is a second, separate setting on `main`: `strict` (require branches up to date), which needs nothing in the repository but serializes merges and stalls `renovate/*` branches that Renovate does not rebase; or a merge queue, which asks no contributor to rebase but requires the required checks to run on `merge_group` events, and **no workflow in this repository has a `merge_group:` trigger** (16 of 16, verified 2026-09-26), so enabling one before that change would leave every queued entry waiting on checks that never run. Both are admin-only. Requiring the context first is still right — it is the half that is free of cost and blocks nothing mergeable today — but it is necessary, not sufficient
