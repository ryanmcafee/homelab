...
- **`homelab verify all --level 0` enforces both**, in `internal/verify/decisions.go`, as the checks `decisions/adr-format` and `decisions/adr-numbers` — plus `decisions/adr-record`, which replaces them when the record cannot be parsed, because a checker that silently read nothing must not emit a green. It reads the repository, needs no cluster, and runs on a pull request twice: `pr-contract.yml` on the head and `verify.yml` on the merge result. **Only the head run blocks.** "Verification claim matches level 0" is `main`'s single required status check; `verify.yml` is not in the required set, so a collision visible only in the merge result is red without being a merge gate. The rebase is what makes it blocking, and the rebase is what the second author is doing anyway. **On a draft or a `renovate/*` head, not even the head run blocks:** `pr-contract.yml`'s `claim` job is `if:`-guarded against both, a job-level skip still publishes a check run whose conclusion is `skipped`, and a `skipped` required context counts as *satisfied* — so on those pull requests the single requirement is met vacuously and no level-0 result gates anything. (Drafts are the benign half: `claim` triggers on `ready_for_review`, so the head run lands before the pull request can merge. `renovate/*` has no such recovery.) Adding `Level 0 (render, schema, gitops, snapshot, policy)` to the required contexts closes both windows for every level-0 check at once — the merge-result window and the exempted-head window, since `verify.yml` carries no draft or Renovate exemption and, with its `pull_request` `paths:` filter removed, runs on every pull request. It needs repository admin; it is tracked separately, and is preferred over building a second merge-result path for this one check
- **Requiring the merge-result check closes the window at run time only, and that is not the whole fix.** `verify.yml` verifies the merge result *as of when it last ran*, and `pull_request` fires on head events only, so a base advance never re-triggers it: the green persists while `main` moves underneath, and once the context is required that stale green is a *satisfied* requirement. The collision above walks straight through — PR X green against `main@T1`, PR Y merges `### ADR-034:` at T2, PR X is one behind and `CLEAN` with a green required level 0, duplicate lands. Making the verdict current is a second, separate setting on `main`, and **both candidates have an unmet in-repository prerequisite** — an earlier revision of this record said `strict` had none, which was wrong. `strict` (require branches up to date) does not merely stall `renovate/*` branches: Renovate stops rebasing a branch once it carries a commit authored outside `gitIgnoredAuthors`, and the manual regeneration this repository documents is what puts one there, so 3 of the 5 open `renovate/*` pull requests are already unrebaseable by Renovate and `strict` would promote them from "out of date" to "unmergeable" (measured 2026-09-26; #252 is `CLEAN` today). That prerequisite is a procedure change, not a credential — the trusted address is a plain workflow-level value applied with `git -c user.email`, so adopting it costs nothing even with the regeneration bot's App secrets unset, which they are (run `36098257343`: every step after the credential check skipped). A merge queue asks no contributor to rebase but runs the required checks on `merge_group` events, and **no workflow in this repository has a `merge_group:` trigger** (16 of 16, verified 2026-09-26), so enabling one first would leave every queued entry waiting on checks that never run; its prep pull request must also make `claim` safe on that event, because both its `if:` and its checkout `ref:` dereference `github.event.pull_request`, which a `merge_group` payload does not carry — get that wrong and the one context required today reports `skipped`, which *satisfies* the requirement rather than wedging visibly. Both settings are admin-only; both prerequisites are agent-doable. Requiring the context first is still right — it is the half that is free of cost and blocks nothing mergeable today — but it is necessary, not sufficient
...
- **What is actually enforced, measured rather than assumed (2026-09-26):** `main` carries exactly one required context, "Verification claim matches level 0", confirmed across all 23 open pull requests. The required set is readable without repository admin — `isRequired(pullRequestNumber:)` on a commit's `statusCheckRollup` answers it for any identity that can see the pull request, where `repos/<owner>/<repo>/branches/main/protection` returns `403 Resource not accessible by integration`. `Level 0 (render, schema, gitops, snapshot, policy)` is present and green on 22 of the 23 and required on none of them, so requiring it would block nothing that is mergeable today. The *write* is admin-only on both enforcement surfaces: the additive `…/branches/main/protection/required_status_checks/contexts` POST and a byte-identical no-op `PUT` to ruleset `12201928` each return `403`, and that ruleset carries no `required_status_checks` rule, so the requirement comes from legacy branch protection only. "Require branches to be up to date" (`strict`) is off, inferred from 16 pull requests behind `main` that report `CLEAN` rather than `BEHIND` (the worst 29 commits behind) — a state legacy protection does not permit with `strict` on — so the merge does not force an update and the merge-result window above is genuinely open
- **How stale the merge-result verdicts actually are (measured 2026-09-26 against `main@918598c3`):** of 24 open pull requests, 20 are behind `main` (83%), 16 are behind *and* `CLEAN` (67%), and every one of those 16 carries a green `Level 0 (render, schema, gitops, snapshot, policy)` whose `completed_at` precedes `main`'s current head commit — up to 19 commits and ~21 h stale. That is the population the "run time only" caveat above applies to, and it is why requiring the context is a floor rather than a fix. `docs/runbooks/verification.md` carries the manual staleness check (`compare/main...<head> --jq .behind_by`) to use until `strict` or a merge queue is on
