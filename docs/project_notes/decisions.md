...
- **The gate now pins the whole contract, not just the type list**: the envelope's `required` array, the stream set (filters, sources, retention, discard, delivery, ordering), the subject grammar and **the payload schemas' own top-level properties**. It rejects status demotion, `dataless` body removal, `requires` *removal*, `producer` reassignment, filter narrowing, retention shortening, a hard-coded tenant in a new type, a `dataschema` that resolves to no file, and — statically — an overlapping stream set
