...
- **2026-09-21** - PR #316: Paperclip `adapters.apiKeys.enabled` (default false) stops the operator from injecting `ANTHROPIC_API_KEY` / `OPENAI_API_KEY` into the pod; subscription tokens stay wired through `adapters.extraSecretEnv` — https://github.com/ryanmcafee/homelab/pull/316
