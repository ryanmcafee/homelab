...
| `gitops/<env>/verified-hosts` | Every external `http(s)` URL a `CronWorkflow` receives as a `url` parameter (the nightly `ingress-verification` run curls each one) names a host this environment serves: an `Ingress` rule host, an `IngressRoute` `Host()` match, or a string in another Application's Helm values. The Application that owns the CronWorkflow's chart is excluded (its `valuesObject` is the URL list itself); in-cluster Service URLs and Argo expressions are ignored; charts no Application deploys are skipped and disclosed in the detail. | Remove the URL of the Application that is disabled in that environment from the list (or enable the Application). |
