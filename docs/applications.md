...
`configuration/versions.yaml` pins, the ingress class it exposes (if any), and how it is
...
- **Ingress** names the Traefik class (`external` or `internal`, see
...
| Application | Source | Version | Ingress | chainsaw e2e | Smoke Job |
| --- | --- | --- | --- | --- | --- |
...
| `istio-base` | `base` | 1.30.5 | — | — | — |
| `istio-cni` | `cni` | 1.30.5 | — | — | — |
| `istio-config` | `charts/istio-config` | git | — | — | — |
| `istiod` | `istiod` | 1.30.5 | — | — | — |
| `kiali` | `kiali-server` | 2.32.0 | internal: servicemesh | — | smoke-kiali |
...
| `lazylibrarian` | `lazylibrarian` | 21.18.2 | internal: lazylibrarian | lazylibrarian | smoke-lazylibrarian |
...
| `nzbget` | `nzbget` | 29.4.2 | internal: nzbget | nzbget | smoke-nzbget |
| `nzbget-config` | `charts/nzbget-config` | git | — | — | — |
| `paperclip` | `charts/paperclip` | git | internal: paperclip | paperclip | smoke-paperclip |
...
| `plex` | `plex-media-server` | 1.6.0 | external: plex | plex | smoke-plex |
| `plex-config` | `charts/plex-config` | git | — | — | — |
| `prowlarr` | `prowlarr` | 21.7.3 | internal: prowlarr | prowlarr | smoke-prowlarr |
| `prowlarr-config` | `charts/prowlarr-config` | git | — | — | — |
| `radarr` | `radarr` | 26.7.2 | internal: radarr | radarr | smoke-radarr |
| `radarr-config` | `charts/radarr-config` | git | — | — | — |
| `renovate` | `renovate` | 46.106.12 | — | — | — |
| `renovate-config` | `charts/renovate-config` | git | — | — | — |
| `sonarr` | `sonarr` | 25.6.3 | internal: sonarr | sonarr | smoke-sonarr |
| `sonarr-config` | `charts/sonarr-config` | git | — | — | — |
| `tautulli` | `tautulli` | 21.18.2 | internal: tautulli | tautulli | smoke-tautulli |
...
- `workflows` (`argo-workflows`) has an internal ingress but **no chainsaw suite and no smoke
  Job**. TODO: add `tests/e2e/argo-workflows/chainsaw-test.yaml` (assert the Application is
  Healthy, then curl `workflows.homelab.local` through Traefik internal) and a `smoke:` block
