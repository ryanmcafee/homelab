...
| Piece | Where |
|---|---|
| Relay + UI at `https://hubble.<DOMAIN>` (internal ingress, `HUBBLE_HOSTNAME`) | Cilium values (`hubble.relay`, `hubble.ui`); Ingress in `charts/cilium-config` |
...
The flow log keeps every flow from or to namespaces `paperclip` and `traefik`, and every
...
render only Services and ConfigMaps; the ServiceMonitors and the UI Ingress are GitOps objects in
...
spans cover it (docs/service-mesh.md). Traffic from outside the mesh (Traefik -> paperclip
waypoint) and every drop stay visible as before. Do not add Cilium L7 policies to enrolled
workloads.
