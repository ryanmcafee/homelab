...
| `cluster/topology.v1.yaml` | The control-plane member count, the etcd quorum formula and the health predicate — shared by the Go CLI and `scripts/cp-storage-migrate.ts` so the rule exists once (ADR-031) |

The event contract is checked by `bun scripts/contract-check.ts` (`task contracts:check`), which
fails on an invalid subject, an unregistered guarantee, or a **breaking** change to a registered
type that did not take a new major version. See `docs/contracts/event-contract.md` for the
reasoning and `docs/project_notes/decisions.md` ADR-025 for the decision record.
...
table matches the stated formula, every permitted topology has a row, and both declared
consumers exist. Each consumer additionally carries its own conformance test asserting that its
