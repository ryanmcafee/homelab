...
| `service-mesh/` | Applications `istio-base`, `istiod`, `istio-cni`, `ztunnel`, `istio-config`, `kiali` Healthy/Succeeded; `ztunnel` and `istio-cni-node` ready on every node; `curl-kiali` gets 200 from `servicemesh.homelab.local/healthz` |
